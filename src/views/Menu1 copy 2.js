...
export default function Menu1() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const url = "http://localhost:8080/members/forum"; // 데이터 가져올 API 엔드포인트
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `데이터 로드 오류: ${errorData.message || response.statusText}`
        );
      }
      ...
      const filteredRows = data
        .filter((item) => {
          ...
          // 지역 필터링
          const matchingArea = !selectedArea || item.city === selectedArea;
          ...
          return (
            dateRange && matchingArea && matchingSubArea && matchingKeyword
          );
        })
        .map((item, index) => ({
          board_id: index + 1,
        ...
        await Promise.all(
          idsToDelete.map(async (id) => {
            const response = await fetch(
              ...
              `http://localhost:8080/members/forum/${id}`,
              {
                method: "DELETE",
                headers: {
                  "Content-Type": "application/json",
  ...
  const handleSave = async () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      showAlert("입력한 내용을 확인하세요.", "warning");
      return;
    }
    ...
    try {
      ...
      await Promise.all(
        rows.map(async (row) => {
          if (selectedRows.has(row.forum_id)) {
            const response = await fetch(`http://localhost:8080/members/forum/${row.forum_id}`, {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
            ...
            if (!response.ok) {
              throw new Error(`저장 오류 ${row.forum_id}`);
            }
          }
        })
      );
          ...
          <TableBody>
            {/* {rows.map((row) => ( */}
              ...
              <React.Fragment key={row.forum_id}>
                <TableRow style={{ borderTop: "1px solid #e4e4e4" }}>
                  <TableCell style={{ width: "50px" }}>
                    <Checkbox
                      checked={selectedRows.has(row.forum_id)}
                      onChange={() => {
                        setSelectedRows((prevSelectedRows) => {
                          const newSelectedRows = new Set(prevSelectedRows);
                          if (newSelectedRows.has(row.forum_id)) {
                            newSelectedRows.delete(row.forum_id);
                          } else {
                            newSelectedRows.add(row.forum_id);
                          }
                          return newSelectedRows;
                        });
                      }}
                    //   onChange={() => handleSelectRow(row.forum_id)}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center" itemType="date">
                    <InputBox
                      ...
                      value={row.date}
                      handleChange={(e) => {
                        handleChangeCell(row.forum_id, "date", e.target.value);
                      }}
                      ref={(el) => {
                        inputRefs.current[`${row.forum_id}-date`] = el;
                      }}
                      customClass=""
                    />
                  </TableCell>
                  <TableCell align="center">
                    <InputBox
                      ...
                      ref={(el) => {
                        inputRefs.current[`${row.forum_id}-elec_total`] = el;
                      }}
                    />
                  </TableCell>
                  <TableCell align="center">
                    <InputBox
                      ...
                      value={row.city}
                      handleChange={(e) =>
                        handleChangeCell(row.forum_id, "city", e.target.value)
                      }
                      ref={(el) => {
                        inputRefs.current[`${row.forum_id}-city`] = el;
                      }}
                    />
                  </TableCell>
                  <TableCell align="center">
                    <InputBox
                      ...
                      initText="선택"
                      ops={
                        areas.find((area) => area.name === row.city)?.subArea ||
                        []
                      }
                      value={row.region}
                      handleChange={(e) =>
                        handleChangeCell(row.forum_id, "region", e.target.value)
                      }
                      ref={(el) => {
                        inputRefs.current[`${row.forum_id}-region`] = el;
                      }}
                    />
                  </TableCell>
                  <TableCell align="center">
                    ...
                    {row.avg_temp ? (
                      <span className="ml-1 text-xs text-zinc-500">°C</span>
                    ) : (
                  ...
                  <TableCell style={{ width: "50px" }}>
                    <IconButton
                      ...
                      size="small"
                      onClick={() => toggleRow(row.forum_id)}
                    >
                ...
                <TableRow>
                  <TableCell
                    colSpan={14}
                    style={{
                      paddingBottom: openRows[row.forum_id] ? 20 : 0,
                      paddingTop: 0,
                      paddingRight: 5,
                      paddingLeft: 50,
                    }}
                      ...
                      <Box sx={{ margin: 1 }}>
                        <div className="wrap">
                          <InputBox
                            ...
                            placeholder="메모"
                            value={row.comment?.content || ""} // comment 객체가 있으면 content 필드를 사용하고, 없으면 빈 문자열
                            handleChange={(e) => {
                              const newContent = e.target.value;
                              handleChangeCell(row.forum_id, "comment", {
                                ...row.comment,
                                content: newContent,
                              });
                            }}
