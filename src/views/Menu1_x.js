...
export default function Menu1() {
                    ...
                    <Box sx={{ height: "100%", width: "100%" }}>
                        {comments.length > 0 ? (
                            comments.map((comment, index) => (
                                <div key={index}>{comment.content || "No content available"}</div>
                            ))
                        ) : (
    ...
    function CustomDetailPanelHeader() {
        ...
        const expandOrCollapseAll = () => {
            ...
            const allRowIdsWithDetailPanels = Object.keys(
        ...
        const Icon = noDetailPanelsOpen ? KeyboardArrowDownIcon : KeyboardArrowUpIcon;
