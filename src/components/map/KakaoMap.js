...
const KakaoMap = ({
    ...
    const [zoomControlAdded, setZoomControlAdded] = useState(false);
    // 색상 배열 정의
    const colorPalette = [
        "#76AAAA",
        "#8ABA75",
    ...
    useEffect(() => {
        ...
        // ZoomControl이 이미 추가되었는지 확인
        // if (!zoomControlAdded) {
        //     const zoomControl = new kakao.maps.ZoomControl();
        //     map.addControl(zoomControl, kakao.maps.ControlPosition.RIGHT);
        //     setZoomControlAdded(true); // ZoomControl이 추가되었음을 표시
        // }
        ...
        const url = `${baseUrl}/electricity?date=${today}&city=${area}&county=${subArea}`;
        getFetchData(url);
    }, [kakao, onMapReady, area, subArea, selectedDate, zoomControlAdded]);
                        ...
                        <div class="unit-wrap">
                            ${
                                Omonth === 6 || Omonth === 7 || Omonth === 8
                                    ? `<div class="unit">최고기온</div> <div class="tem">${
                                          data.max_temp || "N/A"
                                      }<span>ºC</span></div>`
                                    : Omonth === 12 ||
                                      Omonth === 1 ||
                                      Omonth === 2
                                    ? `<div class="unit">최저기온</div>  <div class="tem">${
                                          data.min_temp || "N/A"
                                      }<span>ºC</span></div>`
                                    : `<div class="unit">기온</div>  <div class="tem">${
                                          data.avg_temp || "N/A"
                                      }<span>ºC</span></div>`
                            }
