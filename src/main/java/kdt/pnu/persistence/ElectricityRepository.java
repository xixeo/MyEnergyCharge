...
public interface ElectricityRepository extends JpaRepository<Electricity,Integer>{
	...
	@Query(value = "with daily_windchill as ( "
			...
			+ "     s.min_windchill,     "
			+ "  case "
			+ "  when month(d.date) in (5,6,7,8,9) then exp((d.wind_chill - s.min_windchill)/(s.max_windchill-s.min_windchill)-0.5)  "
			+ "     else exp(-(d.wind_chill - s.min_windchill)/(s.max_windchill-s.min_windchill)-0.5)  "
			...
			+ "d.elec_nonlinear / m.sum_elec_nonlinear *  he.elec_avg  as elec_avg "
